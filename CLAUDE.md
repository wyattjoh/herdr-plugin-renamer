...
- `transcript.rs` — resolve transcript path (glob for Claude/Codex; Copilot
  session-state path; reported session path for Pi) + first-prompt extraction.
  Claude slash-command wrappers are used as a fallback naming prompt, including
  `command-args`, when no normal non-meta user prompt exists; expanded skill
...
- `agent_session` agent label is `claude`, `codex`, `copilot`, or `pi`;
  transcripts: Claude `~/.claude/projects/**/<uuid>.jsonl`, Codex
  `~/.codex/sessions/**/rollout-*<uuid>.jsonl`, Copilot
  `~/.copilot/session-state/<uuid>/events.jsonl`, Pi's reported session path.
